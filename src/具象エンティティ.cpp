...
namespace エンジン
{
	...
	プレイヤー・エンティティ::プレイヤー・エンティティ()
	{
		...
		スプライト_ = dynamic_cast<スプライトコンポーネント*>
			(コンポーネント::コンポーネント生成("スプライトコンポーネント", *this));
		スプライト_->リソース設定(RID_IMG_MAT04_01 + 2);
		追加(スプライト_);

		入力_ = dynamic_cast<入力コンポーネント*>(コンポーネント::コンポーネント生成("入力コンポーネント", *this));
		追加(入力_);

		弾丸_ = dynamic_cast<弾丸コンポーネント*>(コンポーネント::コンポーネント生成("弾丸コンポーネント", *this));
		追加(弾丸_);
	}
	...
	void プレイヤー・エンティティ::更新(float 経過時間)
	{
		...
		if (入力.押し下げ & 入力サービス::マスク_スペシャル) {
			for (int i = 0; i < 6; i++) {
				弾丸_->追加(弾丸サービス::種類::自弾, float2(this->位置_.x + 10 + i * 20, this->位置_.y), this->位置_);
				弾丸_->追加(弾丸サービス::種類::自弾, float2(this->位置_.x - 10 - i * 20, this->位置_.y), this->位置_);
			}
		}
	}
		...
		:生存時間_(0.0f), 弾を撃つまでの時間_(5.0f), 残弾_(3)
	{
		...
		追加(dynamic_cast<弾丸コンポーネント*>(コンポーネント::コンポーネント生成("弾丸コンポーネント", *this)));
	}
	...
	void ザコ１・エンティティ::更新(float 経過時間)
	{
		...
		if (弾を撃つまでの時間_ < 0.0f && 0 < 残弾_) {
			...
			速度 = 速度.正規化() * 100.0f;
			弾丸コンポーネント* 弾丸 = dynamic_cast<弾丸コンポーネント*>(this->コンポーネント検索("弾丸コンポーネント"));
